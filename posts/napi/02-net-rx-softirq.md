...
1. 인터럽트 핸들러가 종료될 때 (`irq_exit()`)
2. Bottom half가 다시 활성화될 때 (`local_bh_enable()`)

softirq의 실행 빈도는 주변 장치 동작(인터럽트 핸들러의 실행 시점과 빈도)과 bottom half의 활성화 빈도에 따라 달라집니다. 이로 인해 프로세스는 매우 긴 시간 지연될 수 있습니다.
