...
*we*는 패킷 수신을 감지하고 처리하기 위해 인터럽트를 사용했습니다. NIC이 패킷을 온보드 메모리에 저장한 후 인터럽트를 발생시키면, CPU가 memory-mapped된 NIC 메모리에서 패킷을 읽어 처리하는 구조입니다. 이는 주변장치가 이벤트를 알리고 CPU가 이를 처리하는 전형적인 이벤트 기반 데이터 처리 모델입니다.
...
# Splitting interrupt work: top-half and bottom-half
...
이렇게 역할을 분리하면 인터럽트 핸들러가 CPU를 모두 사용하는 것을 막고 사용자 프로세스가 실행될 시간을 줄 수 있게 됩니다. 시스템이 수신 속도를 따라가지 못하면 NIC 버퍼가 가득 차 패킷이 드랍되어, 패킷이 *적절히* 버려지게 됩니다.
...
NET_BH는 리눅스 1.1.72에서 `backlog` 큐 길이가 300으로 증가했으며, 이후 리눅스 2.1.79에서 `backlog` 큐 길이를 sysctl을 통해 변경(`netdev_max_backlog`) 할 수 있도록 개선되었습니다.
