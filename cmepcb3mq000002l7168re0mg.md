---
title: "NET_BH"
datePublished: Sun Aug 24 2025 07:01:30 GMT+0000 (Coordinated Universal Time)
cuid: cmepcb3mq000002l7168re0mg
slug: netbh
tags: linux-network, napi, netbh

---

> NET_BH는 1992년에 개발된 기술로, 개발 배경과 관련된 자료가 거의 남아 있지 않습니다. 따라서 아래 내용에는 NET_BH 이후 기술들의 흐름을 바탕으로한 추론이 일부 포함되어 있습니다.
...
*we* 드라이버는 Western Digital의 wd8003 및 wd8013 네트워크 카드를 지원하였고 Ross Biro가 개발했습니다.

## interrupt-driven packet processing

*we*는 패킷 수신을 감지하고 처리하기 위해 인터럽트를 사용했습니다. NIC이 패킷을 CPU 메모리에 DMA 한 후 인터럽트를 발생시키면, 드라이버의 인터럽트 핸들러가 이를 처리하는 구조입니다.

이는 주변장치가 이벤트를 알리고, CPU가 이를 처리하는 전형적인 이벤트 기반 모델에 해당합니다.
...
flowchart TD
    subgraph IRQ["rx interrupt handler"]
        ...
        L -- yes --> A["alloc skb"]
        A --> C[copy buf → skb]
        C --> ENQ[enqueue driver's backlog]
        ENQ --> L

        %% 2) process queued skbs
        L -- no --> S{driver's backlog empty?}
        S -- no --> DQ[dequeue skb]
        DQ --> UP["process skb"]
        UP --> S
        S -- yes --> X[done]
...
*we*의 인터럽트 핸들러는 수신한 패킷들을 (1) 패킷 구조체(skb) 할당 -> (2) 드라이버 버퍼에서 복사 -> (3) 드라이버의 `backlog` 큐에 보관한 후, 큐에서 하나씩 꺼내 처리합니다.

*we*의 인터럽트 핸들러는 수신된 패킷을 다음 단계로 처리했습니다.
- 패킷 구조체(skb)를 할당
- 드라이버 버퍼에서 패킷 구조체로 데이터 복사
- 복사된 패킷을 드라이버의 backlog 큐에 보관
- backlog 큐에서 패킷을 하나씩 꺼내 상위 네트워크 계층으로 전달
...
드라이버가 패킷을 커널로 전달하는 API인 `dev_rint`는 `backlog` 큐에 패킷을 넣은 후, 이후 bottom-half가 실행되 큐에 보관된 패킷들을 처리합니다.

## INET_BH improvements

INET_BH가 패킷 수신 절차를 두 단계로 나눠 지연처리할 수 있게 만들었지만, 패킷 수신량이 늘어나면 top-half의 처리 시간이 길어지고 bottom-half의 실행 시간이 없어지는 interrupt livelock 현상이 발생하게 됩니다.
...
이후 `dev_rint`를 사용하는 드라이버는 점차 줄어들었고, v1.3.6 이후에는 더 이상 어떤 드라이버도 이 API를 사용하지 않게 되었습니다. 이는 버퍼 할당 작업을 커널에서 드라이버로 옮겨, 데이터 복사 과정을 제거함으로써 성능을 개선하기 위함으로 보입니다.
</details>